...
import 'package:youth/ui/views/pages/podCastDetail.dart';
...
class PodCastPage extends StatefulWidget {
  ...
  const PodCastPage({Key key, this.title}) : super(key: key);
        ...
        child: BaseView<PodCastModel>(
          ...
          builder: (context, model, child) => model.loading
              ...
              : SmartRefresher(
                  ...
                  onRefresh: () async {
                    await model.getPodCastList(1, action: "refresh");
                    await Future.delayed(Duration(milliseconds: 1000));
                    _refreshController.refreshCompleted();
                  },
                  onLoading: () async {
                    await model.getPodCastList(model.page + 1, action: "more");
                    await Future.delayed(Duration(milliseconds: 1000));
                    _refreshController.loadComplete();
                  },
                  child: model.podCastModelList.length > 0
                      ? ListView(
                          padding:
                              EdgeInsets.only(left: 15, right: 15, bottom: 30),
                          children: model.podCastModelList.map(
                            (PodCast item) {
                              return InkWell(
                                onTap: () {
                                  // Navigator.push(
                                ...
                                child: Container(
                                  ...
                                  decoration: BoxDecoration(
                                    ...
                                    border: Border.all(
                                      color: podCastColor.withOpacity(.7),
                                    ),
                                  ),
                                  child: Column(
                                    children: <Widget>[
