...
//    return false;
  ...
  Future<bool> fbLogin(BuildContext context, String url) async {
    pr = new ProgressDialog(context, ProgressDialogType.Normal);
...
//          await facebookLogin.logInWithReadPermissions(['email']);
    ...
    switch (result.status) {
      case FacebookLoginStatus.loggedIn:
        ...
        await this.doLogin(context, 0, url, graphResponse.data);
        break;
...
//    }
    ...
    await new Future.delayed(const Duration(seconds: 2));
    pr.hide();
    print("here before false");
    return false;
  }

  Future<void> doLogin(
      BuildContext context, int stype, String url, dynamic user) async {
    ...
    if (stype == 1) {
      print(user);
      response = await _http.post('/api/m/social/login', {
        "email": user.email,
